...
export class GlacierAbility extends Ability {
  ...
  createShaders() {
    ...
    /* ---- the 8-pointed runic matrix on the floor ---- */
    this.runeGeometry = new PlaneGeometry(1, 1);
    this.runeGeometry.rotateX(-Math.PI / 2);
    this.runeMaterial = createGlacialRuneMaterial();
    this.runeMesh = new Mesh(this.runeGeometry, this.runeMaterial);
    this.runeMesh.name = 'GlacialRuneMatrix';
    this.runeMesh.layers.set(LAYER.VFX);
    this.runeMesh.renderOrder = 8;
  ...
  onImpact() {
    ...
    /* ---- 8-Pointed Glacial Rune Matrix Activation ---- */
    if (this.runeMesh) {
      this.runeMesh.visible = true;
      const runeSize = this.radius * 2.3;
      this.runeMesh.scale.set(runeSize, 1, runeSize);
      this._centrePoint(_pos);
      this.runeMesh.position.copy(_pos).setY(0.04);
    }
  ...
  onFade(dt, t) {
    ...
    if (this.runeMesh && this.runeMesh.visible) {
      if (this.runeMaterial?.uniforms) {
        this.runeMaterial.uniforms.uTime.value = this.age;
        this.runeMaterial.uniforms.uProgress.value = fade;
      } else if (this.runeMaterial) {
        this.runeMaterial.opacity = fade * 0.85;
      }
    }
  ...
  onDestroy() {
    ...
    this.field.visible = false;
    this.veil.visible = false;
    if (this.runeMesh) this.runeMesh.visible = false;
  }
