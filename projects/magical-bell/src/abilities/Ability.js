...
export class Ability {
  ...
  get config() {
    return settings[this.element] || settings.ice || {};
  }

  /** Unified 0..1 progression throughout travel, impact, and fade */
  get progress() {
    if (this.phase === AbilityPhase.TRAVEL) return this.u * 0.45;
    if (this.phase === AbilityPhase.IMPACT) return 0.45 + saturate(this.impactTime / this.impactDuration) * 0.35;
    if (this.phase === AbilityPhase.FADE) return 0.80 + saturate(this.fadeTime / this.fadeDuration) * 0.20;
    return 1.0;
  }
  ...
  get targetPosition() {
    return this._targetPosition || new Vector3().copy(this.origin).addScaledVector(this.direction, this.length);
  }
  ...
  get targetPos() {
    return this._targetPos || this.targetPosition;
  }
