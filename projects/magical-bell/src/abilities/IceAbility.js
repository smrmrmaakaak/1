...
import { createFrostLanceMaterial } from '../materials/FrostLanceMaterial.js';
...
export class IceAbility extends Ability {
  ...
  createShaders() {
    ...
    this.spearMaterial = createFrostLanceMaterial();

    // High-poly Diamond Glacial Lance Projectile
    this.spearGroup = new Group();
    this.spearGroup.visible = false;

    // Main Diamond Cut Spear Head
    const spearGeo = new ConeGeometry(0.32, 2.4, 8);
    spearGeo.rotateX(Math.PI / 2);
    const spearMesh = new Mesh(spearGeo, this.spearMaterial);
    this.spearGroup.add(spearMesh);

    // Inner Diamond Core
    const coreGeo = new OctahedronGeometry(0.24, 0);
    coreGeo.scale(1.0, 1.0, 2.2);
    const coreMesh = new Mesh(coreGeo, this.spearMaterial);
    this.spearGroup.add(coreMesh);

    // 6 Orbiting Frost Shards
    this.orbitShards = [];
    for (let i = 0; i < 6; i++) {
      const shardGeo = new DodecahedronGeometry(0.12, 0);
      shardGeo.scale(0.8, 0.8, 1.8);
      const shardMesh = new Mesh(shardGeo, this.spearMaterial);
      this.spearGroup.add(shardMesh);
      this.orbitShards.push(shardMesh);
    }
  ...
  onSpawn() {
    ...
    if (this.spearGroup) {
      ...
      this.spearGroup.position.copy(this.origin).setY(1.1);
    }
  ...
  onTravel(dt) {
    ...
    if (this.spearGroup) {
      this.spearGroup.visible = true;
      this.spearMaterial.uniforms.uTime.value = this.age;

      // Position spear at the front of travel
      const leadPos = _pos.copy(this.origin).addScaledVector(this.direction, this.front).setY(1.0);
      this.spearGroup.position.copy(leadPos);

      // Point forward along direction + fast axial spin
      this.spearGroup.quaternion.setFromUnitVectors(new Vector3(0, 0, 1), this.direction);
      this.spearGroup.rotateZ(this.age * 22.0);

      // 6 Orbiting Frost Shards
      if (this.orbitShards) {
        this.orbitShards.forEach((shard, idx) => {
          const theta = this.age * 14.0 + (idx * Math.PI * 2) / 6;
          const r = 0.6 + Math.sin(this.age * 10.0 + idx) * 0.12;
          shard.position.set(Math.cos(theta) * r, Math.sin(theta) * r, -0.35 + (idx % 2) * 0.35);
          shard.rotation.x = this.age * 8.0 + idx;
          shard.rotation.y = this.age * 9.0 + idx;
        });
      }
    }
  ...
  onDestroy() {
    this._activeCount = 0;
    if (this.spearGroup) this.spearGroup.visible = false;
    for (let v = 0; v < VARIANTS; v++) this.meshes[v].count = 0;
  }
