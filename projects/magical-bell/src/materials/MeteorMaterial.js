        ...
        `#include <emissivemap_fragment>
         {
           ...
           vec3  glow = mix(uColorCrack, uColorHot * 3.2, core * core) * heat * uCrackGlow * 2.2;
           ...
           glow += uColorCrack * rim * uRimHeat * vRockHeat * charge2 * 1.5;

           float lead = pow(clamp(dot(normalize(vRockNormalW), uHeading), 0.0, 1.0), uLeadSharp);
           glow += uColorHot * lead * uLead * vRockHeat * charge2 * 2.5;

           glow *= uGlow * 1.8;

           // Smooth HDR tone curve allowing intense radiant bloom without blowout
           glow /= 1.0 + glow * 0.08;
