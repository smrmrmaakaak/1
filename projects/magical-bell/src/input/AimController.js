...
export class AimController extends EventEmitter {
  ...
  _resolve() {
    ...
    // Robust fallback: if pointer not yet moved, aim forward with default comfortable range
    if (!this.direction.lengthSq()) {
      this.direction.set(0, 0, 1);
    }
    this.valid = true;
    this.distance = MathUtils.clamp(this.distance || (c.range * 0.6 || 15.0), Math.max(0.2, c.minRange || 1.0), Math.max(0.4, c.range || 25.0));
  }
