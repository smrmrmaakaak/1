...
export class DecalSystem {
  ...
  /**
   * @param {number|number} typeOrX   DecalType.* OR number x
   * @param {THREE.Vector3|number} posOrZ position OR number z
   * @param {object|number} optionsOrRadius options OR radius
   * @param {number} [maybeType]
   * @param {number} [maybeLife]
   */
  spawn(typeOrX, posOrZ, optionsOrRadius = {}, maybeType, maybeLife) {
    let type = typeOrX;
    let position = posOrZ;
    let options = optionsOrRadius;

    // Overload: spawn(x, z, radius, type, life)
    if (typeof typeOrX === 'number' && typeof posOrZ === 'number') {
      type = typeof maybeType === 'number' ? maybeType : DecalType.SCORCH;
      position = new Vector3(typeOrX, 0, posOrZ);
      options = {
        radius: typeof optionsOrRadius === 'number' ? optionsOrRadius : 3,
        life: typeof maybeLife === 'number' ? maybeLife : 3
      };
    } else if (typeof typeOrX !== 'number' && typeof posOrZ === 'number') {
      // Fallback
      type = DecalType.SCORCH;
    }
