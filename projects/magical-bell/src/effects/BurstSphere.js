...
export class BurstSystem {
  ...
  /** Alias for spawn supporting flexible parameter signatures */
  trigger(posOrMode, endRadiusOrPos = 3, maybeMode = BurstMode.FIRE, life = 0.6, color = null) {
    if (typeof posOrMode === 'number' && endRadiusOrPos && typeof endRadiusOrPos === 'object') {
      return this.spawn(posOrMode, endRadiusOrPos, typeof maybeMode === 'object' ? maybeMode : {});
    }
    const position = (posOrMode && typeof posOrMode === 'object') ? posOrMode : new Vector3();
    const endRadius = typeof endRadiusOrPos === 'number' ? endRadiusOrPos : 3.5;
    const mode = typeof maybeMode === 'number' ? maybeMode : BurstMode.FIRE;
    return this.spawn(mode, position, { endRadius, life });
  }
