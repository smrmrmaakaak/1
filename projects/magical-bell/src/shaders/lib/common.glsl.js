...
float softFade(sampler2D sceneDepth, vec2 screenUV, float fragViewZ, float near, float far, float fadeDist) {
  vec2 uvClamped = clamp(screenUV, 0.001, 0.999);
  vec4 packedColor = texture2D(sceneDepth, uvClamped);
  float packed = unpackRGBAToDepth(packedColor);
  if (packed <= 0.0001 || packed >= 0.9999) return 1.0;
  float sceneViewZ = perspectiveDepthToViewZ(packed, max(near, 0.05), max(far, 500.0));
  float diff = fragViewZ - sceneViewZ;
  return clamp(diff / max(fadeDist, 0.01), 0.0, 1.0);
}

/** Standard Schlick-ish rim term. */
float fresnelTerm(vec3 viewDir, vec3 normal, float power, float scale) {
  float d = clamp(1.0 - abs(dot(normalize(viewDir), normalize(normal))), 0.0, 1.0);
  return clamp(scale * pow(d, max(power, 0.1)), 0.0, 4.0);
}
...
vec2 screenUVFromClip(vec4 clipPos) {
  return clamp((clipPos.xy / max(clipPos.w, 1e-4)) * 0.5 + 0.5, 0.0, 1.0);
}
