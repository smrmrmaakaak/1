...
export class Editor {
  ...
  constructor(hooks = {}) {
    ...
    // Everything starts collapsed, top-level folders included.
    this.gui.foldersRecursive().forEach((folder) => folder.close());
    // Hide by default to keep the screen clean for the defense game mode
    this.gui.hide();
  }
  ...
  toggle() {
    if (this.gui._hidden) {
      this.gui.show();
      this.hooks.onToast?.('VFX Editor opened');
    } else {
      this.gui.hide();
      this.hooks.onToast?.('VFX Editor hidden');
    }
  }
