...
async def verify_bottom_pricing():
    async with async_playwright() as p:
        ...
        # Use context with ignore_https_errors and clear cache
        context = await browser.new_context(viewport={"width": 1600, "height": 960})
        page = await context.new_page()

        print("[+] Accessing http://localhost:5174 ...")
        ...
        if await gallery_btn.count() > 0:
            ...
            await page.screenshot(path="qa_vintage_textures/04_local_pricing_card.png")
            print("[+] 04_local_pricing_card.png captured!")
