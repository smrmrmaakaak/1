...
f_item_title = ImageFont.truetype(FONT_DOHYEON, 20)
f_item_desc = ImageFont.truetype(FONT_JUA, 16)
...
print("Pre-rendering 8-card scenes with luxury dark glassmorphism (No Neon)...")
scene_assets = []
...
for idx, sc in enumerate(SCENES):
    ...
    # 4. Pre-render 8 Detailed Subtitle Cards Strip
    bullets = sc["bullets"]
    strip_w = strip_box_w
    ...
    for b_title, b_desc in bullets:
        ...
        c_h = 10 + 24 + 6 + len(desc_lines) * 23 + 10
        card_items.append({
            "title": b_title,
            "lines": desc_lines,
            "height": c_h
    ...
    for ci in card_items:
        ch = ci["height"]
        st_draw.rounded_rectangle([(0, cur_y), (strip_w, cur_y + ch)], radius=14, fill=(20, 28, 23), outline=(255, 255, 255, 22), width=1)
...
print(f"Prepared {len(scene_assets)} 8-card scene assets.")
...
def render_dynamic_frame(asset, progress, global_f):
    ...
    # 2. Right Card: Upward Scrolling 8-Card Subtitles Animation
...
print("Setting up FFmpeg rendering process for 장터의 노래 master video...")
ffmpeg_cmd = [
    'ffmpeg', '-y',
    '-f', 'rawvideo',
    '-vcodec', 'rawvideo',
    ...
    '-i', '-',
    '-i', LOCAL_AUDIO,
    '-filter_complex', '[1:a]afade=t=in:st=0:d=1.0,afade=t=out:st=155.0:d=3.5[a]',
    '-map', '0:v',
    '-map', '[a]',
    '-c:v', 'libx264',
    '-preset', 'veryfast',
